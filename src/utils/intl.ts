...
//   >,
// ): string | React.ReactNodeArray | React.ReactNode => {
//   // return intl.formatHTMLMessage(descriptor, values as any);
...
const pleaseSelect = (name?: string) => {
  return il8n({ id: 'msg.pleaseSelect' }, { name: name ?? '' });
};
/**
...
const pleaseEnter = (name?: string) => {
  return il8n({ id: 'msg.pleaseEnter' }, { name: name ?? '' });
};
...
export { il8n, intl, pleaseEnter, pleaseSelect, showErrorMessage };
